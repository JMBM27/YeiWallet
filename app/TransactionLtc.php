...
{
    protected $table = 'transaction_ltc';
    ...
    public static function actTransactionReceived($txs,$id,$address){
        $txant=DB::table('transaction_ltc')->where('usuario_id',$id)->where('tipo',2)->take(25)->get();
        foreach($txs as $tx) {
            $esta=false;
    ...
    public static function actTransactionSent($txs,$id,$address){
        $txant=DB::table('transaction_ltc')->where('usuario_id',$id)
                                           ->where('tipo',1)
