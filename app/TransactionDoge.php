...
class TransactionDoge extends Model
{
    protected $table = 'transaction_doge';
   ...
   public static function actTransactionReceived($txs,$id,$address){
        $txant=DB::table('transaction_doge')->where('usuario_id',$id)->where('tipo',2)->take(25)->get();
        foreach($txs as $tx) {
            $esta=false;
    ...
    public static function actTransactionSent($txs,$id,$address){
        $txant=DB::table('transaction_doge')->where('usuario_id',$id)
                                           ->where('tipo',1)
        ...
        foreach($txs as $tx) {
            for($i=0;$i<count($txant);$i++){
                if($txant[$i]->txid == $tx->txid){
                    if($tx->confidence>0.90){
                        $update=TransactionDoge::find($txant[$i]->id);
    ...
    public static function getTransaction($skip,$take,$id){
        $tx=DB::table('transaction_doge')->where('usuario_id',$id)
                                        ->orderBy('fecha',	'desc')
    ...
    public static function total(){
        $total=Session::get('TDoge');
        return $total;
    }
    public static function ctotal(){
        $total = DB::table('transaction_doge')->count();
        Session::put('TDoge', $total);
    }
}
